				...
				<form action="{{route('messages.update',$message->id)}}"
					...
					<div class="card-body">
						<div class="form-group">
							...
							value="{{ isset($message) ? $message->name : ''}}">
						</div>
						<div class="form-group">
							<label>{{__('site.phone')}}</label>
							<input type="text" class="form-control" name="phone" required
							value="{{ isset($message) ? $message->phone : ''}}">
						</div>
						<div class="form-group">
							<label>{{__('site.email')}}</label>
							<input type="text" class="form-control" name="email" required
							value="{{ isset($message) ? $message->email : ''}}">
						</div>
						<div class="form-group">
							<label>{{__('site.message')}}</label>
							<textarea class="form-control" name="description" required>{{old('description',$message->description) }}</textarea>
						</div>
